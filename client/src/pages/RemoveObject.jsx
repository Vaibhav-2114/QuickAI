  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4 text-slate-700'>
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <FileUploadField file={input} onChange={setInput} accept='image/*' theme='blue' required />

        <p className='mt-6 text-sm font-medium'>Describe object to remove</p>

        <textarea onChange={(e) => setObject(e.target.value)} value={object} rows={4} className=' w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border border-gray-300' placeholder='e.g., watch or spoon — single object name only' required />
      ...
      <div className='w-full max-w-lg p-4 bg-white rounded-lg flex flex-col border border-gray-200 min-h-96'>
        ...
        {loading ? (
          <div className='flex-1 flex flex-col justify-center items-center gap-5 text-blue-500'>
          ...
          <>
            <OutputActions content={content} type='image' theme='indigo' filename='object-removed' />
